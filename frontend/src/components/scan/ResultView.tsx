...
export default function ResultView({ result, onRetry, onConfirmCandidate }: ResultViewProps) {
  ...
  const ViewWrapper = ({ children }: { children: React.ReactNode }) => (
    <div className="w-full h-full bg-ink flex flex-col items-center justify-center p-6 text-center relative overflow-hidden">
    ...
    return (
      <ViewWrapper>
        <div className="relative mb-6">
          ...
          <CheckCircle2 className="w-20 h-20 text-success relative z-10 drop-shadow-[0_0_10px_var(--color-success)]" />
        </div>
        <h2 className="text-3xl font-bold text-white mb-2 tracking-wide">Match Found!</h2>
        <p className="text-lg text-bone/70 mb-8 max-w-md font-light">
        ...
        <div className="w-full max-w-sm flex flex-col gap-3 relative z-10">
          <button 
            ...
            className="w-full bg-success/10 hover:bg-success/20 text-success border border-success/30 font-semibold py-3.5 px-6 rounded-xl flex items-center justify-center gap-2 transition-colors shadow-[0_0_15px_var(--color-success)]"
          >
            <Eye className="w-5 h-5" /> View Pet Profile
          </button>
          <button 
            onClick={() => navigate('/sightings/new')}
            className="w-full bg-surface/5 hover:bg-surface/10 text-bone/90 border border-white/5 font-medium py-3.5 px-6 rounded-xl flex items-center justify-center gap-2 transition-colors"
          >
  ...
  if (result.state === 'AMBIGUOUS' && result.candidates) {
    return (
      <div className="w-full h-full bg-ink flex flex-col items-center justify-start p-6 pt-12 overflow-y-auto relative">
        <div className="absolute top-0 inset-x-0 h-96 bg-gradient-to-b from-white/[0.02] to-transparent pointer-events-none"></div>
        
        <div className="relative mb-4 shrink-0">
          <div className="absolute inset-0 bg-marigold/20 blur-xl rounded-full"></div>
          <AlertCircle className="w-16 h-16 text-marigold relative z-10" />
        </div>
        
        <h2 className="text-2xl font-bold text-white mb-2">Multiple Similar Profiles</h2>
        <p className="text-bone/70 mb-8 text-center max-w-md font-light">
        ...
        <div className="w-full max-w-md flex flex-col gap-4 mb-8 relative z-10">
          {result.candidates.map(candidateId => {
            ...
            if (!pet) return null;
            return (
              <div key={candidateId} className="bg-surface/5 p-4 rounded-2xl border border-white/5 shadow-xl flex items-center justify-between gap-4 transition-transform hover:scale-[1.02]">
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-sm text-bone/50 capitalize">{pet.breed}</p>
                  </div>
                </div>
                <button 
                  onClick={() => onConfirmCandidate(candidateId)}
                  className="bg-marigold/10 hover:bg-marigold/20 text-marigold border border-marigold/20 px-5 py-2.5 rounded-xl font-medium transition-colors text-sm whitespace-nowrap"
                >
                  Confirm
                </button>
              </div>
        ...
        <button 
          onClick={onRetry}
          className="text-bone/50 hover:text-bone/90 font-medium py-3 px-4 transition-colors mt-auto relative z-10"
        >
    ...
    return (
      <ViewWrapper>
        <div className="relative mb-6">
          ...
          <HelpCircle className="w-20 h-20 text-accent relative z-10" />
        </div>
        ...
        <div className="w-full max-w-sm flex flex-col gap-3 relative z-10">
          <button 
            ...
            className="w-full bg-accent hover:bg-accent-hover text-white font-semibold py-3.5 px-6 rounded-xl flex items-center justify-center gap-2 transition-colors shadow-[0_4px_14px_var(--color-marigold)]"
          >
    ...
    return (
      <ViewWrapper>
        <div className="relative mb-6">
          ...
          <AlertTriangle className="w-20 h-20 text-danger relative z-10" />
        </div>
        <h2 className="text-2xl font-bold text-white mb-2 tracking-wide">Scan Unclear</h2>
        <p className="text-bone/70 mb-6 max-w-md font-light">
          We couldn't get a clear read of the nose pattern. {result.errorDetails && <span className="block mt-2 font-medium text-bone/90">Issue: {result.errorDetails}</span>}
        </p>
        <div className="bg-surface/5 border border-white/5 text-bone/90 p-5 rounded-2xl text-sm mb-8 max-w-md text-left shadow-xl relative z-10">
  ...
  if (result.state === 'SYSTEM_FAILURE') {
    return (
      <ViewWrapper>
        <div className="relative mb-6">
          <div className="absolute inset-0 bg-alert/20 blur-xl rounded-full"></div>
          <ServerCrash className="w-20 h-20 text-alert relative z-10" />
        </div>
        <h2 className="text-2xl font-bold text-white mb-2 tracking-wide">Service Unavailable</h2>
        <p className="text-bone/70 mb-8 max-w-md font-light">
          Our identification servers are currently unreachable. This is a system issue, not a problem with your scan.
          {result.errorDetails && <span className="block mt-4 text-xs text-alert/70 font-mono bg-surface/5 border border-white/5 p-3 rounded-xl">{result.errorDetails}</span>}
        </p>
