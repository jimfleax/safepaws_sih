...
export default function ProcessingView({ phase }: ProcessingViewProps) {
  ...
  return (
    <div className="w-full h-full bg-[var(--color-ink)] flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="relative mb-12 flex flex-col items-center justify-center">
        ...
        <div className="relative w-48 h-48 rounded-[2rem] bg-white/5 shadow-[inset_0_2px_1px_rgba(255,255,255,0.05),0_20px_40px_rgba(0,0,0,0.4)] border border-white/5 flex items-center justify-center overflow-hidden">
          
          {/* Rendered Nose-print proxy */}
          ...
          <div 
            className="absolute left-0 right-0 h-16 bg-gradient-to-b from-transparent via-[var(--color-marigold)]/[0.15] to-transparent pointer-events-none motion-reduce:hidden"
            style={{ top: `${scanPos}%`, transform: 'translateY(-50%)' }}
          >
             <div className="absolute top-1/2 left-0 right-0 h-[1px] bg-[var(--color-marigold)]/30 shadow-[0_0_8px_var(--color-marigold)]" />
          </div>
        </div>
      </div>
      
      <h2 className="text-xl font-medium tracking-wide text-[var(--color-bone)] text-center mb-2">
        {message}
      </h2>
      
      <p className="text-[var(--color-bone)]/50 text-sm mb-10 font-light">
        Biometric verification
      </p>
      
      {/* Subtle Material Progress Bar */}
      <div className="w-48 h-1 bg-white/10 rounded-full overflow-hidden shadow-[inset_0_1px_2px_rgba(0,0,0,0.5)]">
        <div 
          className="h-full bg-[var(--color-bone)]/80 rounded-full transition-all duration-1000 ease-out"
